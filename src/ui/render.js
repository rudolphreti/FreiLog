...
export const renderApp = (root, state) => {
  ...
  if (!classSettingsView) {
    classSettingsView = createClassSettingsOverlay({ children: sortedChildren });
  } else {
    classSettingsView.update({ children: sortedChildren });
  }
  ...
  if (!appShell) {
    ...
    if (classSettingsView && actions?.settingsButton) {
